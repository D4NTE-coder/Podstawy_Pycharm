...
@app.route("/login")
def login_user():
    session.pop('token_info', None)  # Wyczyść token przed logowaniem
    print(f"Sesja przed logowaniem: {session}")  # Debugowanie
    return login()
...
@app.route('/logout')
def logout_user():
    print(f"Przed wylogowaniem: {session}")  # Debugowanie
    response = logout()
    print(f"Po wylogowaniu: {session}")  # Powinno być {}
    return response
...
def recently_played():
    ...
    try:
        ...
        if not results or 'items' not in results or len(results['items']) == 0:
            return jsonify({'error': 'Brak danych. Spotify może nie rejestrować historii.'})
